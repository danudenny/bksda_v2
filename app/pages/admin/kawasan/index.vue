...
const {
  ...
  deleteKawasanCategory: deleteApi,
  loading,
} = useApi();
...
async function deleteCategory(id: string) {
  ...
  if (response.success) {
    categories.value = categories.value.filter((s: any) => s.id !== id);
...
<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-2">
          Manage conservation categories and locations
        </p>
      </div>
      <Button
        @click="router.push('/admin/kawasan/create')"
    ...
    >
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                <Button
                  variant="ghost"
                  size="sm"
                  @click="router.push(`/admin/kawasan/${item.id}`)"
      ...
      <div v-if="paginated.length === 0" class="px-6 py-12 text-center">
      ...
      <div class="flex space-x-2">
        ...
        <div class="flex space-x-1">
          <Button
            ...
            size="sm"
            @click="goToPage(page)"
            :class="[page === currentPage ? 'bg-primary text-white dark:bg-green-500' : '']"
          >
            {{ page }}
          </Button>
        </div>
