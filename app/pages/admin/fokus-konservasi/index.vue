...
const { getSpecies, deleteSpecies: deleteApi, loading } = useApi();
...
const filteredSpecies = computed(() => {
  return speciesList.value.filter((item) => {
    return (
      ...
      item.latinName.toLowerCase().includes(searchQuery.value.toLowerCase())
    );
  });
});

const totalPages = computed(() =>
  Math.ceil(filteredSpecies.value.length / itemsPerPage.value),
);
...
async function fetchSpecies() {
  ...
  if (response.success) {
    ...
    toast.error(response.message || "Failed to fetch species");
  }
}
...
async function deleteSpecies(id: string) {
  ...
  if (response.success) {
    speciesList.value = speciesList.value.filter((s) => s.id !== id);
    toast.success("Species deleted successfully");
  } else {
    toast.error(response.message || "Failed to delete species");
  }
}
...
<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-2">
          Manage conservation focus species
        </p>
      </div>
      <Button
        @click="router.push('/admin/fokus-konservasi/create')"
    ...
    >
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
            <tr
              v-for="species in paginatedSpecies"
              :key="species.id"
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                <Button
                  ...
                  size="sm"
                  @click="
                    router.push(`/admin/fokus-konservasi/${species.id}`)
                  "
      ...
      <div
        v-if="paginatedSpecies.length === 0"
        class="px-6 py-12 text-center"
      >
        <p class="text-gray-600 dark:text-gray-400">
          {{ searchQuery ? "No species found" : "No species yet" }}
        </p>
      </div>
    </div>
    ...
    <div v-if="totalPages > 1" class="mt-6 flex items-center justify-between">
      <div class="text-sm text-gray-600 dark:text-gray-400">
        ...
        {{
          Math.min(currentPage * itemsPerPage, filteredSpecies.length)
        }}
        of {{ filteredSpecies.length }} species
      </div>
