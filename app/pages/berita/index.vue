<template>
    <NuxtLayout name="archive">
        ...
        <template #header>
            <h1
                class="text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl mb-6"
            >
                Berita & Kegiatan <br />
                <span
                    class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 to-teal-400"
                    >BKSDA Sumsel</span
                >
            </h1>
        </template>
        ...
        <template #description>
            <p class="mx-auto max-w-2xl text-lg leading-8 text-emerald-100/80">
                Ikuti perkembangan terbaru konservasi, kegiatan lapangan, dan
                upaya pelestarian alam di Sumatera Selatan.
            </p>
        </template>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <template v-if="statusCategories === 'pending'">
                        <div
                            v-for="i in 4"
                            :key="i"
                            class="h-9 w-24 rounded-full bg-gray-200 animate-pulse"
                        ></div>
                    </template>
        ...
        <div>
            ...
            >
                <div
                    v-for="n in 6"
                    :key="n"
                    class="rounded-2xl overflow-hidden bg-white shadow-sm border border-gray-100 h-[400px]"
                >
                    <div class="h-48 bg-gray-200 animate-pulse"></div>
                    <div class="p-6 space-y-4">
                        <div
                            class="h-4 w-1/3 bg-gray-200 rounded animate-pulse"
                        ></div>
                        <div
                            class="h-6 w-3/4 bg-gray-200 rounded animate-pulse"
                        ></div>
                        <div class="space-y-2">
                            <div
                                class="h-3 w-full bg-gray-200 rounded animate-pulse"
                            ></div>
                            <div
                                class="h-3 w-5/6 bg-gray-200 rounded animate-pulse"
                            ></div>
                        </div>
                    </div>
                </div>
            ...
            <div
                v-else-if="formattedPosts.length === 0 && !isLoading"
                class="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-300"
            >
                <div
                    class="rounded-full bg-white p-4 shadow-sm ring-1 ring-gray-900/5"
                >
                    <svg
                ...
                <p class="text-gray-500 text-center max-w-sm mt-2">
                    Belum ada berita untuk kategori yang Anda pilih saat ini.
                    Coba reset filter atau pilih kategori lain.
                </p>
                <button
                    @click="resetFilters"
                    class="mt-6 text-sm font-semibold text-emerald-600 hover:text-emerald-500"
                >
                    Hapus semua filter
                </button>
            </div>
                ...
                <TransitionGroup name="list">
                    ...
                    >
                        <NuxtLink :to="post.href" class="flex flex-col h-full">
                            <div
                                class="relative aspect-[16/9] overflow-hidden sm:aspect-[2/1] lg:aspect-[3/2]"
                            >
                                <img
                                    :src="
                                        post.imageUrl ||
                                ...
                                />
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-gray-900/60 via-transparent to-transparent opacity-60 group-hover:opacity-40 transition-opacity"
                                ></div>
                            ...
                            <div class="flex flex-1 flex-col p-6">
                                <div
                                    class="flex items-center gap-x-3 text-xs text-gray-500 mb-3"
                                >
                                    <time
                                ...
                                <h3
                                    class="text-xl font-bold leading-snug text-gray-900 group-hover:text-emerald-600 transition-colors line-clamp-2 mb-3"
                                >
                                    {{ post.title }}
                                </h3>

                                <p
                                    class="mt-auto text-sm leading-relaxed text-gray-600 line-clamp-3"
                                >
                                    {{ post.description }}
                                </p>

                                <div
                                    class="mt-6 flex items-center gap-3 pt-4 border-t border-gray-100"
                                >
                                    <div
                                        class="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 shrink-0"
                                    >
        ...
        <div v-if="hasNextPage" class="mt-16 flex justify-center">
            ...
            >
                <span
                    v-if="isLoading"
                    class="absolute inset-0 flex items-center justify-center bg-emerald-700"
                >
                    <svg
        ...
        <div v-else-if="formattedPosts.length > 0" class="mt-12 text-center">
            <p class="text-sm text-gray-400 italic">
                Anda sudah melihat semua berita
            </p>
        </div>
    </NuxtLayout>
</template>
...
import { MoveDownIcon, UserIcon } from 'lucide-vue-next';
...
const formattedPosts = computed(() => {
    return posts.value.map((post) => ({
        ...
        date: post.createdAt
            ? format(parseISO(post.createdAt), 'dd MMMM yyyy', { locale: id })
            : '-',
