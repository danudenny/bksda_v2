<template>
  <div class="flex justify-center">
    <div class="inline-flex items-center gap-4 sm:gap-6 rounded-2xl bg-gradient-to-r from-emerald-900/40 via-teal-900/40 to-emerald-900/40 px-6 py-4 backdrop-blur-sm ring-1 ring-white/10">
      <!-- Icon -->
      <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/50 flex-shrink-0">
        <Users class="h-5 w-5 text-white" />
      </div>
      <!-- Stats -->
      <div class="flex items-center gap-4 sm:gap-6 divide-x divide-white/10">
        <!-- Today -->
        <div class="flex items-center gap-2">
          <TrendingUp class="h-4 w-4 text-emerald-400 hidden sm:block" />
          <div>
      ...
      <div class="flex items-center gap-1.5 flex-shrink-0">
        ...
        <span class="text-xs text-emerald-400 font-semibold hidden sm:inline">Live</span>
      </div>
    </div>
  </div>
</template>
...
import { Users, TrendingUp, Calendar, Eye } from "lucide-vue-next";
