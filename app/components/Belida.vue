<template>
  <section class="relative overflow-hidden bg-gradient-to-b from-slate-50 via-white to-slate-50 py-16 sm:py-24">
    ...
    <div class="absolute inset-0 -z-10">
      <div class="absolute right-0 top-0 h-96 w-96 rounded-full bg-emerald-100/40 blur-3xl filter"></div>
      <div class="absolute left-0 bottom-0 h-96 w-96 rounded-full bg-blue-100/40 blur-3xl filter"></div>
    </div>

    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <!-- Header Section -->
      <div class="mx-auto max-w-3xl text-center">
        <div class="inline-flex items-center gap-2 rounded-full bg-emerald-600/10 px-4 py-2 ring-1 ring-inset ring-emerald-600/20">
        ...
        <h2 class="mt-6 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">
          Akses <span class="bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Tautan</span> Cepat
        </h2>
        <p class="mt-4 text-lg text-slate-600">
          Akses cepat ke berbagai sistem dan platform penting untuk mendukung pekerjaan Anda
        </p>
      </div>
      ...
      <div class="mx-auto mt-12 max-w-2xl">
        <div class="relative group">
          <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-5">
            <Search class="h-5 w-5 text-slate-400 transition-colors duration-300 group-focus-within:text-emerald-600" aria-hidden="true" />
          </div>
          <input
            v-model="searchQuery"
            type="search"
        ...
        <!-- Search Results Info -->
        <div v-if="searchQuery && filteredLinks.length > 0" class="mt-3 text-center">
          <p class="text-sm text-slate-600">
            Ditemukan <span class="font-semibold text-emerald-600">{{ filteredLinks.length }}</span> tautan
          </p>
        </div>
      </div>

      <!-- Links Grid -->
      <div class="mx-auto mt-16 grid max-w-none grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <NuxtLink
        ...
        >
          ...
          <div class="absolute top-4 right-4 flex items-center gap-2">
            <span
              ...
              :title="link.status === 'online' ? 'Online' : link.status === 'offline' ? 'Offline' : 'Mengecek...'"
            >
              <span
                v-if="link.status === 'online'"
          ...
          <!-- Icon -->
          <div class="mb-4 inline-flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/50 transition-transform duration-300 group-hover:scale-110">
            <AppWindow class="h-7 w-7 text-white" aria-hidden="true" />
          </div>

          <!-- Content -->
          <div class="flex-grow">
            <h3 class="text-lg font-bold text-slate-900 group-hover:text-emerald-600 transition-colors duration-300">
              {{ link.title }}
            </h3>
          ...
          <!-- Footer -->
          <div class="mt-4 flex items-center justify-between pt-4 border-t border-slate-100 transition-colors duration-300 group-hover:border-emerald-500">
            <span class="text-sm font-semibold text-emerald-600">
              Kunjungi
            </span>
            <ArrowRightCircleIcon class="h-5 w-5 text-emerald-600 transition-transform duration-300 group-hover:translate-x-1" />
          </div>

          <!-- Hover Gradient Bar -->
          <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-500 to-teal-500 transform scale-x-0 transition-transform duration-300 group-hover:scale-x-100"></div>
        </NuxtLink>
        ...
        >
          ...
          <p class="mt-2 text-slate-600">
            Tidak ada tautan yang cocok dengan pencarian "<span class="font-semibold text-emerald-600">{{ searchQuery }}</span>".
          </p>
          <button
            @click="searchQuery = ''"
      ...
      <div v-if="filteredLinks.length > 4" class="mt-12 text-center">
        ...
        >
          <template v-if="!isExpanded">
            <span>Tampilkan Semua Tautan ({{ filteredLinks.length }})</span>
            <ChevronDown class="h-5 w-5 transition-transform duration-300 group-hover:translate-y-0.5" />
          </template>
          <template v-else>
            <span>Tampilkan Lebih Sedikit</span>
            <ChevronUp class="h-5 w-5 transition-transform duration-300 group-hover:-translate-y-0.5" />
          </template>
        </button>
      </div>
      ...
      <div class="mt-8 text-center">
        <p class="text-sm text-slate-500">
          Total <span class="font-semibold text-slate-700">{{ allLinks.length }}</span> tautan tersedia
        </p>
      </div>
    </div>
  </section>
