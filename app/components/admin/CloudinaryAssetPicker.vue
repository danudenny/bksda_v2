<template>
    <Dialog v-model:open="open">
        <DialogContent
            class="sm:max-w-4xl h-[85vh] flex flex-col p-0 gap-0 overflow-hidden"
        >
            <DialogHeader
            ...
            <div
                class="p-4 bg-gray-50 dark:bg-gray-900/50 flex flex-wrap items-center gap-2 shrink-0"
            >
                ...
                <div
                    class="h-6 w-px bg-gray-300 dark:bg-gray-700 mx-2 hidden sm:block"
                ></div>
            ...
            >
                ...
                <div
                    ...
                    class="absolute inset-0 p-4 grid gap-4 overflow-hidden bg-white dark:bg-gray-950 z-10"
                    :style="{
                        gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`,
                    }"
                >
                ...
                <div
                    v-if="loading && assets.length > 0"
                    class="flex justify-center py-4 absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white dark:from-black to-transparent"
                >
                    <div
                        class="bg-white dark:bg-gray-800 shadow-md rounded-full px-4 py-1 flex items-center gap-2 text-xs font-medium"
                    >
            ...
            <DialogFooter
                class="p-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shrink-0"
            >
...
import { Loader, Upload } from 'lucide-vue-next';
import {
    computed,
    nextTick,
    onBeforeUnmount,
...
const totalRows = computed(() =>
    Math.ceil(assets.value.length / columns.value)
);
...
async function fetchAssets(reset = false) {
    ...
    try {
        ...
        const res = await request<{ assets: any[]; nextCursor: string | null }>(
            '/cloudinary/assets',
            { params }
        );
...
function getThumbUrl(url: string) {
    if (url.includes('/upload/')) {
        return url.replace(
            '/upload/',
            '/upload/c_fill,w_300,h_300,q_auto:low,f_auto/'
        );
    }
    return url;
}
